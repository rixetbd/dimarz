        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('backend.leads.index')}}">Leads Data</a></li>
                </ol>
            </div>
            <div class="col-sm-6 d-flex justify-content-end">
                <a class="btn btn-primary" href="{{route('backend.leads.create')}}"> <i class="fa fa-plus"></i> Upload Leads Data</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    $('#dataTableStyle').DataTable({
        ajax: {
            url: `{{route('backend.leads.autoleads')}}`,
            dataSrc: ''
        },
        columns: [{
                data: null,
    ...
    function cat_distroy(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let formUrlData = `{{route('backend.leads.destroy')}}`;
                $.ajax({
                    type: "POST",
                    url: `${formUrlData}`,
                    data: {
