...
@section('page_title', 'All Testimonial')
        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{route('backend.testimonial.index')}}">Testimonial</a></li>
                </ol>
            </div>
            <div class="col-sm-6 d-flex justify-content-end">
                <a class="btn btn-primary" href="{{route('backend.testimonial.create')}}"> <i class="fa fa-plus"></i> Add Testimonial</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    $('#dataTableStyle').DataTable({
        ajax: {
            url: `{{route('backend.testimonial.alltestimonial')}}`,
            dataSrc: ''
        },
        columns: [{
                data: null,
    ...
    function cat_distroy(id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                let formUrlData = `{{route('backend.testimonial.destroy')}}`;
                $.ajax({
                    type: "POST",
                    url: `${formUrlData}`,
                    data: {
                    ...
                    error: function (request, status, error) {
                        notyf.error('Testimonial Delete Unsuccessfully!');
                    }
                });
            }
        })
